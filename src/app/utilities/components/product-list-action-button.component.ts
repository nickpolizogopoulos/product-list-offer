...
@Component({
    selector: 'app-product-list-action-button',
    standalone: true,
    template: `

        @if (buttonType() === 'add') {
            <button #addProductTooltip class="products-icon-btn" [class]="buttonClass()" type="button">
        ...
        @else {
            <button class="products-icon-btn" [class]="buttonClass()" type="button">
                <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="currentColor" viewBox="0 0 16 16">
...
export class ProductListActionButtonComponent {
    ...
    private initializeTooltip(): void {
        ...
        const tooltipContent = 
              language.isGreek()   ? 'Προσθέστε νέο προϊόν'
            : language.isEnglish() ? 'Add a new product'
            : language.isSpanish() ? 'Añadir un nuevo producto'
            : language.isFrench()  ? 'Ajouter un nouveau produit'
            : language.isItalian() ? 'Aggiungi un nuovo prodotto'
            : language.isRussian() ? 'Добавить новый продукт'
            : '새 제품 추가'
